...
// Query runs query (use $1, $2, ... placeholders) and returns every
// matching row as the full-state Observation. Compare the result against
// an expected []Row with furumai.Diff/furumai.ThenEqual (wrap the
// expectation in furumai.AnyOrder if row order isn't significant).
